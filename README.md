...
When deploying a symlink, tdm backs up the original version of the file. When tdm vacates, it restores these locations to their previous state.
...
- `files`: This is where the majority of your dotfiles are located. tdm uses the relative path from `files/` to find where the symlink should be created. `(files/relative_path` creates a symlink `$HOME/relative_path` -> `files/relative_path`)
